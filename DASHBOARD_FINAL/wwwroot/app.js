document.addEventListener('DOMContentLoaded', () => {
    ...
    const btnConnectSim = document.getElementById('btnConnectSim');
    if (btnConnectSim) {
        btnConnectSim.addEventListener('click', () => {
            window.chrome.webview.postMessage({ action: isSimConnected ? 'disconnectSim' : 'connectSim' });
            const dashSc = document.getElementById('dashSimStatus');
            if (dashSc && !isSimConnected) {
                dashSc.innerText = 'Connecting natively...';
                dashSc.style.color = '#FACC15';
            }
        });
    }
    ...
    window.chrome.webview.addEventListener('message', event => {
        ...
        switch (payload.type) {
            ...
            case 'simConnectStatus':
                ...
                if (dashSc) dashSc.innerText = payload.status;
                
                isSimConnected = payload.status.includes('Connected') || payload.status.includes('Linked');
                if (btnConnectSim) {
                    btnConnectSim.innerText = isSimConnected ? 'Unlink Simulator' : 'Link Simulator';
                    btnConnectSim.style.backgroundColor = isSimConnected ? '#DC2626' : '#4A90E2';
                }

                if (isSimConnected) {
                    sc.className = 'status-pill success';
                    if (dashSc) dashSc.style.color = '#34D399';
                } else {
                    sc.className = 'status-pill error';
                    if (dashSc) dashSc.style.color = '#F87171';
                }
                break;
            case 'telemetry':
            ...
            case 'fetchStatus':
                ...
                break;
            case 'groundOpsReady':
                if (btnStartGroundOps) btnStartGroundOps.style.display = 'block';
                break;
            case 'flightCancelled':
                ...
                if (btnStartGroundOps) btnStartGroundOps.style.display = 'none';
                break;
            case 'flightData':
