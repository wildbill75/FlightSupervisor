...
- [ ] **TICKET 2 : Création de la jauge WASM (C++)**
  ...
  - Implémentation du `SimConnect_Open` côté Gauge et création de la zone mémoire partagée (`Client Data Area`).
...
- [ ] **TICKET 4 : Écoute du Client Data Area (C#)**
  - Implémentation des appels `MapClientDataNameToID` et de l'event handler `OnRecvClientData` dans le `SimConnectService.cs` principal.
